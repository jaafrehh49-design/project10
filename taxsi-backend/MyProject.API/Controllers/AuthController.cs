...
namespace MyProject.API.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public IActionResult Login(LoginDto dto)
        {
            ...
            // 👇 تسجيل دخول وهمي
            if (dto.Email == "test@test.com" && dto.Password == "1234")
            {
                return Ok(new
                {
                    token = "fake-token",
        ...
        public IActionResult Register(RegisterDto dto)
        {
            ...
            return Ok(new { message = "تم إنشاء الحساب 🔥 (تجريبي)" });
        }

        [HttpPost("send-reset-token")]
        public IActionResult SendResetToken(ForgotPasswordDto dto)
        {
            return Ok(new { message = "تم إرسال رابط إعادة التعيين (تجريبي)" });
        }

        [HttpPost("reset-password")]
        public IActionResult ResetPassword(ResetPasswordDto dto)
        {
            return Ok(new { message = "تم تغيير كلمة المرور 🔥 (تجريبي)" });
        }
    }
}